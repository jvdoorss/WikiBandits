...
import torch
from torch.nn import Sequential, Linear, Sigmoid, CrossEntropyLoss, Softmax
...
class Arm:
    ...
    def estimate(self,url,subject): 
        ''' Predictor to map the state of the Arm and the context-variables to a prediction of the reward'''
        pass
    def action(self,url,subject,log):
        '''Download and additional operation to be performed after the arm is picked, but before its reward is calculated'''
        pass
...
class Classifier(Arm):
    ...
    def _evaluate(self,url,subject):
        '''helper function to create an intermediate representation for both the context and its estimated effect'''
